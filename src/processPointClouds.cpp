...
template<typename PointT>
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint, bool removeRoof)
{
    ...
    // Removes a hardcoded box (x:[-1.5,2.6] y:[-1.7,1.7] z:[-1,-0.4]) meant to
    // strip an ego-vehicle's own roof-rack/rooftop-sensor returns from the
    // original car dataset this pipeline was built for. Guarded by
    // removeRoof since that box sits exactly where a person standing close
    // to a non-vehicle-mounted sensor would appear, silently deleting them.
    if (removeRoof)
    {
